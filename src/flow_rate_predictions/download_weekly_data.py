...
# Use this file to download mobile phone gps data from the University of Wisconsin-Madison Lab
import requests
import os
import pandas as pd
import numpy as np
    ...
    try:
        if scale == "ct2ct":
            for i in range(20):
                ...
                r = requests.get(
                    url=f"https://raw.githubusercontent.com/GeoDS/COVID19USFlows-{repo}/master/weekly_flows/{scale}/{year}_{month}_{day}/weekly_{scale}_{year}_{month}_{day}_{i}.csv")
                with open(f"{output_folder}/{scale}/{year}_{month}_{day}/weekly_{scale}_{year}_{month}_{day}_{i}.csv",
                          'wb') as file:
